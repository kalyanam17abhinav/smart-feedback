...
import { getFeedback, deleteFeedbackById } from "@/lib/api";
...
const Admin = () => {
  ...
  const deleteFeedback = async (id: string) => {
    try {
      ...
      // Update stats
      const deletedItem = feedback.find(item => item.id === id);
      if (deletedItem) {
        setStats(prev => ({
          ...prev,
          total: prev.total - 1,
          positive: prev.positive - (deletedItem.sentiment === 'Positive' ? 1 : 0),
          negative: prev.negative - (deletedItem.sentiment === 'Negative' ? 1 : 0),
          neutral: prev.neutral - (deletedItem.sentiment === 'Neutral' ? 1 : 0),
          registered: prev.registered - (deletedItem.user_id ? 1 : 0),
          anonymous: prev.anonymous - (!deletedItem.user_id ? 1 : 0),
        }));
      }
